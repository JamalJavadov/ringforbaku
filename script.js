...
// İnstagram və WhatsApp linklərini yenilə
...
// ===== YENİ: Səhifələmə (Pagination) Tənzimləmələri =====
const productsPerPage = 9; // SƏHİFƏ BAŞINA MƏHSUL SAYI (Dəyişə bilərsiniz, məsələn 12). Dizaynın sabit qalması üçün bu rəqəmi dəyişməyin.
let currentPage = 1;
let filteredProducts = [...products]; // Başlanğıcda bütün məhsullar göstərilir


// ===== MƏHSUL FUNKSİONALLIĞI (Yenilənmiş) =====

// 1. Məhsul Kartlarını Ekrana Çap Etmək (Artıq səhifəyə görə göstərir)
    ...
    productsToShow.forEach(product => {
        ...
        card.innerHTML = `
            <div class="product-image-container">
                <img src="${product.images[0]}" alt="Məhsul şəkli" loading="lazy">
            </div>
...
// 2. YENİ: Səhifələmə Düymələrini Yaratmaq
function setupPagination() {
    paginationContainer.innerHTML = '';
    const pageCount = Math.ceil(filteredProducts.length / productsPerPage);

    if (pageCount <= 1) return; // Əgər 1 səhifə və ya daha azdırsa, nömrələri göstərmə
...
// 4. Teqə Klikləmə Hadisəsi (Yenilənmiş)
...
// 5. Məhsulları Filterləmək (Yenilənmiş)
function filterProducts() {
    ...
    currentPage = 1; // Filterləmə zamanı həmişə birinci səhifəyə qayıt
    displayProducts();
    setupPagination();
}

// 6. Modal Pəncərə Funksiyaları (Dəyişiklik yoxdur)
...
// ===== SƏHİFƏ YÜKLƏNƏNDƏ İŞƏ DÜŞƏN KODLAR =====
...
// ===== 3D/2D SAYT MÜHƏRRİKİ (Yenilənmiş Məsafə ilə) =====

if (window.matchMedia('(min-width: 1025px)').matches) {
    let scene, camera, webGLRenderer, css3DRenderer;
    let contentGroup = new THREE.Group();
    let sections = [];
    const sectionDistance = 1200; // Artırılmış və sabit məsafə
    ...
    function createParticles() {
        ...
        for (let i = 0; i < count * 3; i++) { positions[i] = (Math.random() - 0.5) * 3000; }
        particlesGeometry.setAttribute('position', new THREE.BufferAttribute(positions, 3));
        const particlesMaterial = new THREE.PointsMaterial({ size: 1.5, color: 0xaaaaee, blending: THREE.AdditiveBlending, transparent: true });
   ...
   function createNavigation() {
    ...
    // ===== TELEFON VƏ PLANŞET ÜÇÜN 2D TƏCRÜBƏ (Dəyişiklik yoxdur) =====
    document.getElementById('content-sections').style.display = 'block';
    document.body.style.overflow = 'auto';
    const hamburger = document.querySelector('.hamburger');
    const navLinks = document.querySelector('.nav-links');
